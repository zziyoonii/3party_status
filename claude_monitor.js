...
export class ClaudeStatusMonitor {
  ...
  constructor(sequelize) {
    ...
    this.api_url = settings.CLAUDE_STATUS_API_URL;
    this.timeout = settings.CLAUDE_STATUS_TIMEOUT * 1000; // 밀리초로 변환
    ...
    try {
      ...
      if (statusResponse.status === 200) {
        ...
        if (indicator === 'none') {
          ...
          error_message = `Claude Status: ${indicator}`;
        ...
        status = ServerStatus.DOWN;
        error_message = `Claude Status API error: ${statusResponse.status}`;
      }
    } catch (error) {
      response_time_ms = Date.now() - startTime;
      ...
      if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
        status = ServerStatus.DOWN;
        error_message = 'Claude Status API request timeout';
      } else if (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
        status = ServerStatus.DOWN;
        error_message = 'Claude Status API connection error';
      } else {
        status = ServerStatus.DOWN;
        error_message = `Unexpected error checking Claude Status: ${error.message}`;
      }
    }
  ...
  async analyzeErrorLevel(record) {
    ...
    // Claude 서버의 최근 시간 윈도우 내의 실제 오류 개수 확인 (UNKNOWN 제외)
  ...
  async createAlertIfNeeded(record, error_level) {
    ...
    const now = new Date();
    const message = `Claude 서비스 상태 이상 감지: ${record.status}. 오류 메시지: ${record.error_message || 'N/A'}`;

    const latestUnresolved = await Alert.findOne({
      where: { resolved: 0, message: { [Op.like]: '%Claude%' } },
    ...
    if (latestUnresolved && latestUnresolved.error_level === error_level) {
      await Alert.update(
        ...
        { where: { resolved: 0, message: { [Op.like]: '%Claude%' }, id: { [Op.ne]: latestUnresolved.id } } }
      );
  ...
  async monitor() {
    const record = await this.checkClaudeStatus();
    ...
    // 서버 정상 복구 시 알림 자동 해결
    await this.alertManager.autoResolveAlerts('claude', record);
    
    // Slack 알림 확인 및 발송 (다운 상태가 5분 이상 지속된 경우)
    await this.slackNotifier.checkAndNotify('claude', record);
    
    // CRITICAL 이슈가 5분 이상 지속된 경우 Slack 알림 발송
    await this.slackNotifier.checkAndNotifyCritical('claude', record);
